		...
		if(!window.slider) var slider={};slider.data=[
            <?php for ($i = 1; $i < $adjuntoImagenes->count(); $i++) { $a = $adjuntoImagenes->offsetGet($i);?> 
               {"id":"slide-img-<?php echo $i;?>","client":"<?php echo $a->get_NOM();?>","desc":"<?php echo $a->get_LINK();?>"}<?php if ($i<($adjuntoImagenes->count()-1)){ echo ",";}?>
            <?php }?>	                          		
			];
		</script>		
        <h1 class="titolHome"><?php echo $contingutSeleccionat->get_TITOLPAGINA();?></h1>
        </div><!-- Final WRAPPER -->		
		<!-- Final SLIDESHOW -->